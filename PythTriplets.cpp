...
#endif
	ftop[n]=len;
	for(int i=0;i<=n;i++)edge[i][n]=edge[n][i]=false;
	for(int i=0;i<len;i++)
	...
	while(len>1)
	{
		...
		while(path[len-1]>=n)
		{
			...
			if(f)
			{
				...
				assert(i<ftop[super]);
				if(0==(i&1))for(int j=0;j<=i;j++)path[len++]=flower[super][j];
				else 
				{
					path[len++]=flower[super][0];
					for(int j=ftop[super]-1;j>=i;j--)path[len++]=flower[super][j];
				}
			}
			else
			{
				int next=s;
				if(len)next=path[len-1];
				int i;
				for(i=0;i<ftop[super]&&!edge[flower[super][i]][next];i++);
				assert(i<ftop[super]);
				if(0==(i&1))for(int j=i;j>=0;j--)path[len++]=flower[super][j];
				else
				{
					for(int j=i;j<ftop[super];j++)path[len++]=flower[super][j];
					path[len++]=flower[super][0];
				}
			}
		}
		assert(len>0&&path[len-1]<n);
		if(f)match[v]=path[len-1],match[path[len-1]]=v;
...
bool findPath(const int s,int n)
{
	...
	while(top>0)
	{
		...
		printf("Expore %d\n",v);
#endif
		if(color[v]==ODD)continue;
		assert(color[v]==EVEN);
		while(pedge[v]<n&&(!edge[v][pedge[v]]||removed[pedge[v]]||color[pedge[v]]==ODD))
			pedge[v]++;
		if(pedge[v]==n)continue;
		stack[top++]=v;instack[v]=true;
		int u=pedge[v]++;
		if(color[u]==WHITE)
		{
			father[u]=v;
		...
		else
		{
			...
			else
			{
				...
				// The just contracted new vertex forms a odd cycle with some already visited vertex
				/*
                    3---------------0       
                    |               |              
					1 ------------- 2 
			...
			if(top>0)father[n]=stack[top-1];
			else father[n]=-1;
			stack[top++]=n;instack[n]=true;
			color[n]=EVEN;
			pedge[n]=0;
			removed[n]=false;
			root[n]=-1;
...
{
	for(int i=0;i<2*n;i++)memset(edge[i],false,sizeof(bool)*2*n);
	for(int i=0;i<n;i++)
	...
	int re=0;
	for(int i=0;i<n;i++)myindex[i]=n-1-i;
...
class PythTriplets {
	...
	{
		vector<llint> all=parse<llint>(accumulate(stick.begin(),stick.end(),string(""))," ");
		...
		return generalMaximumMatching(n,edge);
	}
};
