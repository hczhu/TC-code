...
int Greedy(int n) {
  ...
  while(cnt < n) {
    ...
    Rep(i, n) if (!visited[i]) {
      if (next == -1 || make_pair(dis[v][next], next) > make_pair(dis[v][i], i)) {
        next = i;
      }
    }
    gmin[v] = dis[v][next];
...
class GreedyTravelingSalesman {
 public:
  int worstDistance(vector <string> thousands, vector <string> hundreds, vector <string> tens, vector <string> ones) {
    ...
    vector<int> value;
    Rep(i, n) Rep(j, n) {
      dis[i][j] = 1000 * Value(thousands[i][j]) + 100 * Value(hundreds[i][j]) + 10 * Value(tens[i][j]) + Value(ones[i][j]);
      value.push_back(dis[i][j]);
      value.push_back(dis[i][j]);
      value.push_back(dis[i][j] - 1);
    }
    ...
    ans = Greedy(n);
    debug(pick[1][2]);
    memcpy(org,pick, sizeof(org));
    memcpy(gg, gmin, sizeof(gg));
    debug(org[1][2]);
    int low[50][50], high[50][50];
    Rep(i, n)Rep(j, n) Rep(k, SZ(value)) {
       int x = value[k];
      if ( x < 1 || x > 9999) continue;
      if (!org[i][j] && (x > gg[i]||x < gg[i] - 1)) {
        continue;
      }
    // cerr << i << " " << j << "=" << x << endl;
      if (org[i][j] && x <= dis[i][j]) {
        continue;
      }
      //cerr << i << " " << j << " " << x << endl;
      swap(dis[i][j], x);
      ans = max(ans, Greedy(n));
      swap(dis[i][j], x);
    }
    return ans;
  }
