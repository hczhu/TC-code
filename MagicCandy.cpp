...
map<int, int> sq;
int mysqrt(int m) {
      map<int,int>::iterator itr = sq.lower_bound(m);
      if (itr == sq.end() || itr->first > m) --itr;
      return itr->second;
}
class MagicCandy {
 public:
   int solve(int n) {
     ...
     int m = n - (int)sqrt(n);
     int ans= solve(m);
     return ans + mysqrt(ans);
   }
  int whichOne(int n) {
    // n = 1000000000;
    for (int t = 1, ins = 1; t*t <= n; ++t, ++ins) {
      sq[t*t - ins +1 ] = ins;
    }
    return solve(n);
  }
...
int main() {
  ...
  int n;
  cin>>n;
  cout<< ___test.whichOne(n) <<endl;
  cout<< test_sol(n) <<endl;
}
// END CUT HERE
