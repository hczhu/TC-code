...
class EllysJuice {
 public:
  vector <string> getWinners(vector <string> players) {
    ...
    sort(All(players));
    do {
      int mount[2] = {(1<<10),1<<10};
      map<string,int> drink;
      int idx = 0;
      Rep(i, SZ(players)) {
        idx^=1;
        drink[players[i]]+=mount[idx]>>1;
        mount[idx] >>= 1;
      }
      vector<pair<int, string> > all;
      for (typeof(drink.begin()) itr = drink.begin(); itr != drink.end(); ++itr) {
        all.push_back(make_pair(-(itr->second), itr->first));
      }
      sort(All(all));
      if (all.size() == 1 || all[0].first != all[1].first) {
        winner.insert(all[0].second);
        debug(all[0].second);
        debug(all[0].first);
      }
    } while(next_permutation(All(players)));
    return vector<string>(All(winner));
  }
